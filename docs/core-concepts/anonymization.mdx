...
steps:
  ...
  - name: Configure anonymization
    run: xata clone config --source-url $PROD_DB_URL
  - name: Create anonymized replica
    run: xata clone start --source-url $PROD_DB_URL --target-url ${{ secrets.XATA_BRANCH_URL }}
