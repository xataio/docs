...
```sh
xata roll start .xata/migrations/001_alter_order_items.yaml
```

This command starts the migration but leaves it uncompleted and allows you to roll back if needed.

## 9. Roll back the migration

To test rolling back, run:
...
## 12. (Optional) Add to CI/CD with GitHub Actions
...
- Use `xata roll start` to apply a specific migration
